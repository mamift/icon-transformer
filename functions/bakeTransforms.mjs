  ...
  $(groups).each((i, group) => {
     ...
     $svg.find('path').each(function () {
         if ($(this).attr('transform') != undefined) {
             var newPath = svgpath($(this).attr('d')).transform($(this).attr('transform'))
                 .round(10)
                 .toString();
             $(this).attr(
                 'd',
     ...
     $svg.find('[points][transform]').each(function () {
       ...
       translates = translates.map(
         translate => translate.split(',').map(
           coord => coord.trim()
         )
       );

       var translateX = translates.reduce((value, translate) => parseFloat(value) + parseFloat(translate[0]), 0);
       var translateY = translates.reduce((value, translate) => parseFloat(value) + parseFloat(translate[1]), 0);

       var pointGroups = $(this).attr('points').match(/([\d\.]+[ ,]+[\d\.]+)/g);

       var points = pointGroups.map((group) => {
         var coords = group.split(' ');
         return (parseFloat(coords[0]) + translateX)
           + ' '
